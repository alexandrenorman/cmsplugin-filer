...
class FilerImagePlugin(CMSPluginBase):
    ...
    def _get_thumbnail_options(self, context, instance):
        ...
        if instance.image:
            ...
            if not height and width:
                ...
                height = int(
                    float(width)
                    * float(instance.image.height)
                    / float(instance.image.width)
                )
    ...
    def get_thumbnail(self, context, instance):
        if instance.image:
            return instance.image.file.get_thumbnail(
                self._get_thumbnail_options(context, instance)
            )
    ...
    def icon_src(self, instance):
        if instance.image:
            if (
                getattr(settings, "FILER_IMAGE_USE_ICON", False)
                and "32" in instance.image.icons
            ):
            ...
            return static(
                "filer/icons/missingfile_%sx%s.png"
                % (
                    32,
                    32,
                )
